# VPN Subscription

Подписка с конфигурациями, прошедшими проверку доступности через сеть компьютера, на котором запускается сборщик. Список обновляется после каждого завершённого прогона.

## Добавление в клиент

Скопируйте этот адрес в поле добавления подписки вашего VPN/прокси-клиента:

```text
https://raw.githubusercontent.com/ak-freedom/vpn-subscription/main/subscription.txt
```

Формат — Base64 от текстового списка URI, по одной конфигурации в строке. Подходит клиентам с поддержкой V2Ray-совместимых Base64-подписок и соответствующих протоколов. Наличие ядра Xray или sing-box само по себе не гарантирует поддержку такого импорта: проверяйте возможности приложения.

Клиент должен поддерживать протоколы и параметры конкретного ключа, включая транспорт, TLS или Reality. Для клиентов, принимающих только Clash YAML либо JSON-конфигурацию sing-box, потребуется преобразование формата.

## Что содержится в репозитории

- `subscription.txt` — готовая Base64-подписка. Исходные URI и названия ключей сохраняются.
- `status.json` — время последней проверки в UTC, количество кандидатов и рабочих конфигураций, число раундов, ошибки загрузки источников и распределение рабочих ключей по протоколам.

Сборщик обрабатывает VLESS, VMess, Trojan, Shadowsocks, SOCKS, WireGuard, Hysteria2, Hysteria, TUIC, AnyTLS и ShadowsocksR в пределах возможностей инструментов проверки. В конкретном обновлении остаются только прошедшие проверку конфигурации; фактический состав указан в `status.json`.

## Как получается подписка

1. Сборщик загружает исходные подписки и локальные списки ключей, разбирает URI и Base64; для части форматов используется PulseConfigs.
2. Удаляет повторы, сохраняя различия в портах, SNI, путях и других параметрах подключения.
3. Предварительно проверяет TCP-доступность адресов VLESS и ставит доступные первыми в очередь. Неудача TCP не исключает ключ; UDP-протоколы этот этап пропускают.
4. Выполняет настоящие HTTPS-запросы через каждую конфигурацию. Основные протоколы проверяет xray-knife, дополнительные протоколы и варианты URI — Mihomo.
5. Обычно выполняются три раунда; каждый ключ должен пройти все раунды и все заданные адреса проверки. Фактическое число раундов записано в `status.json`. По умолчанию проверяется `https://www.gstatic.com/generate_204`, от которого требуется ответ HTTP 204.
6. Прошедшие конфигурации сортируются по медианной задержке раундов, кодируются в Base64 и публикуются одним коммитом вместе со сводкой результата.

Запуск и проверка выполняются локально, а GitHub хранит готовый результат. Автозапуск настроен после входа в Windows, не чаще одного раза за календарный день; возможны ручные прогоны. Постоянного облачного расписания нет.

Завершённый прогон без рабочих ключей публикует пустую подписку. Если часть источников недоступна, публикуется проверенный результат из остальных, а число ошибок указывается в `status.json`. Прерванный прогон не публикуется. При сбое выгрузки предыдущая версия на GitHub сохраняется — ориентируйтесь на время проверки в `status.json`.

## Ограничения

Успешная проверка подтверждает доступность заданного HTTPS-адреса через ключ в конкретной сети и в момент проверки. Доступность из другой сети, скорость передачи файлов, все сайты и все режимы UDP отдельно не подтверждаются. Публичные серверы могут перестать работать между обновлениями; их операторы не контролируются сборщиком.

## Инструменты

- [PulseConfigs](https://github.com/drmikecrypto/PulseConfigs) — разбор подписок и структурные проверки.
- [xray-knife](https://github.com/lilendian0x00/xray-knife) — HTTPS-проверки через прокси-ядра.
- [Mihomo](https://github.com/MetaCubeX/mihomo) — дополнительный разбор URI и HTTPS-проверки.
- [VLESS Collector](https://github.com/dequar/deqwl/blob/main/collector_gui.py) — источник идеи предварительной TCP-проверки; его удаление повторов и определение страны не используются.
